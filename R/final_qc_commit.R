...
get_issue_commits <- function(owner, repo, issue) {
  updates <- check_if_there_are_update_comments(owner, repo, issue$number)
  if (!updates) {
    # return init qc commit
    get_init_qc_commit(owner, repo, issue$number)
  }
  else {
    # get comments
...
get_final_issue_qc_commit <- function(owner, repo, issue) {
  issue_commits <- get_issue_commits(owner, repo, issue)
}
